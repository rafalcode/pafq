...
#define ENDB2 "\n+" // Some SRA fastqs appear to add the ID onto the + line
...
{
    char *id; /* id line */
    char *id2; /* because SRA repeat something after the + signi: most likely it is the ID line again, but one can't be sure */
...
void outnwn(bva_t **paa, unsigned ecou, char *fname) /* output in new filename */
{
    int i, j;
...
bva_t *crea_bva(void)
{
    ...
    sr->id2=malloc(HBUF*sizeof(char));
...
char fillidtonl(unsigned char *bf, size_t compfsz, size_t *bfidx, bva_t *pa) /* read chars from bf (becuase file has been decompressed) to bva until end of line */
{
    size_t currbfidx=*bfidx;
    int c;
    char STATE=0;
...
char fillid2tonl(unsigned char *bf, size_t compfsz, size_t *bfidx, bva_t *pa) /* read chars from bf (becuase file has been decompressed) to bva until end of line */
{
    size_t currbfidx=*bfidx;
    int c;
    unsigned cidx=0, idbuf=HBUF;
...
void processfq(char *fname, unsigned char *bf, size_t compfsz, unsigned char acval)
{
    ...
    for(;;) {
        ...
        retval=fillid2tonl(bf, compfsz, &bfidx, paa[ecou]);
        if(retval==2)
            break;
        else if(retval) {
            printf("Error on reading the plus-sign line, no \'+\' symbol forthcoming\n");
            exit(EXIT_FAILURE);
        }
    ...
    printf("%.*s: Totalseqs: %u. Totalbases: %zu. Mx sqsz: %u. Min sqsz: %u. Max. qualval=%c . Min qualval= %c\n", (int)(fnp-fname), fname, ecou, smmry.totb, smmry.mxnbps, smmry.mnnbps, smmry.mx, smmry.mn);
    // HTML version:
