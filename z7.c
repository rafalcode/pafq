/* z6: inflates a gzipped file. valgrind says OK, if gzip is smaller than uncompressed file, which is what you'd expect
...
int defla(unsigned char **srcbf, FILE *dest, size_t *bfsz, int level)
{
    int ret, flush;
    ...
    if(!mark) { // gzip file is bigger than uncompressed file ... happens with tiny files. Let's normalise so that valgind does not complain.
        tbfsz=strm.total_out;
        tbf=realloc(tbf, tbfsz*sizeof(unsigned char));
    }
