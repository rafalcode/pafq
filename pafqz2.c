...
typedef struct /* oberall summary ... for all of the sequences */
{
    size_t totb;
    size_t nsqs;
} osmmry_t;
...
void processfq(char *fname, unsigned char *bf, size_t compfsz, unsigned char acval, osmmry_t *osmm)
{
...
int main(int argc, char *argv[])
{
    ...
    for(i=0;i<opstru.numinps;i++) {
        ...
        processfq(opstru.inputs[i], fbf, compfsz, acval, &osmm);
        fclose(fpa);
    }
    printf("</table>\n</p>\n");
    printf("<p>Overall number of sequences: %zu\n</p>", osmm.nsqs);
    printf("<p>Overall number of bases: %zu\n</p>", osmm.totb);
    printf("</body>\n</html>\n");
